...
namespace SeriesServer.Controllers
{
    ...
    public class SeriesController : ControllerBase
    {
        ...
        private static readonly bool ImplementsLoadingMultipleSeries = true;

        private static readonly bool ImplementsEditSeries = true;

        private static readonly bool ImplementsBrowse = true;

        private static readonly bool ImplementsSearch = true;

        private static readonly bool ImplementsMeta = true;
        ...
        /// </summary>
        /// <param name="names">Names of series to load.</param>
        ...
        /// </summary>
        /// <param name="names">Names of series metadat to load.</param>
        /// <returns>List of series.</returns>
        ...
        /// <summary>
        /// Returns a specific vintage of a series.
        /// </summary>
        /// <param name="name">The name of series to load.</param>
        /// <param name="timeStamp">The representing the vintage.</param>
        ...
        [HttpGet("loadvintage")]
        public ActionResult<Series> LoadVintage([Required][FromQuery(Name = "n")] string name, DateTime timeStamp)
        {
            lock (m_lock)
            {
                ...
                // Find the series with the requested timestamp
                // We use the LastModifiedTimeStamp here already part of the metadata, but this could be any other timestamp that is used to identify revisions

                Series? series = items.FirstOrDefault(x => timeStamp >= (DateTime)x.MetaData[LastModifiedTimeStamp]);
                ...
                return new Series(meta, series.Values, series.Dates);
            }
        }
        ...
        public ActionResult<VintageEvent[]> LoadVintageTimeStamps([Required][FromQuery(Name = "n")] string name)
        {
            lock (m_lock)
            {
                if (!m_dataBase.TryGetValue(name, out var items))
                ...
                {
                    s.MetaData.TryGetValue("RevisionLabel", out var revisionLabel);
                    return new VintageEvent() { TimeStamp = (DateTime)s.MetaData[LastModifiedTimeStamp], Label = (string?)revisionLabel };
                }).ToArray();
            }
        }
        ...
        public ActionResult<DateTime> CreateSeries([FromBody] Series series, [FromQuery] DateTime? lastModified, [FromQuery] bool forceReplace)
        {
            ...
            lock (m_lock)
            {
                ...
                m_dataBase[name] = [newSeries];
                foreach (var p in m_seriesMetaData)
                {
                    if (p.Key == "Other")
                    {
                        var seriesRowList = p.Value.Groups[0].Series.ToList();
                        seriesRowList.Add(new SeriesRow(name, 0, false, false, [name]));
        ...
        {
            lock (m_lock)
                return m_dataBase.TryGetValue(name, out var series) ? series[^1] : null;
        }
        ...
        public ActionResult<List<object>> LoadTree([FromQuery] string? reference)
        {
            lock (m_lock)
            {
                if (string.IsNullOrWhiteSpace(reference))
                    return m_browseDataBase[string.Empty];

                if (m_browseDataBase.TryGetValue(reference, out List<object>? value))
                    return value;
            }
        ...
        public ActionResult<SeriesList> ListSeries([Required][FromQuery] string reference)
        {
            lock (m_lock)
            {
                if (!string.IsNullOrWhiteSpace(reference) && m_seriesMetaData.TryGetValue(reference, out SeriesList? value))
                {
        ...
        public ActionResult<IEnumerable<Dictionary<string, object>>> SearchSeries([Required][FromQuery] string query)
        {
            ...
            lock (m_lock)
            {
                foreach (var result in m_dataBase.Values)
                {
                    var series = result[^1];
                    if (series.MetaData["Description"] is string name && name.Contains(query, StringComparison.OrdinalIgnoreCase))
                        list.Add(series.MetaData);
                }
            }
        ...
        private static readonly Dictionary<string, IReadOnlyList<Series>> m_dataBase = new(StringComparer.OrdinalIgnoreCase)
        {
            {
                "pltour0001",
                [new Series
                (
                    new Dictionary<string, object>()
                    {
                        ...
                        { "LastReleaseEventTime",  new DateTime(2020, 01, 01, 0, 0, 0, DateTimeKind.Utc) },
                        { "NextReleaseEventTime",  new DateTime(2020, 02, 01, 0, 0, 0, DateTimeKind.Utc) },
                        { "LastModifiedTimeStamp", new DateTime(2020, 01, 01, 0, 0, 0, DateTimeKind.Utc) },
                    },
                    [
            ...
            },
            {
                ...
                (
                    new Dictionary<string, object>()
                    {
                        ...
                        { "Frequency", "annual" },
                        { "EntityType", "TimeSeries" },
                        { "LastModifiedTimeStamp", new DateTime(2020, 05, 01, 0, 0, 0, DateTimeKind.Utc) },
                    },
                    [
            ...
            {
                // This series supports revision history. This is indicated by the "StoresRevisionHistory" metadata property.
                "setrad1588",
                [
                    new Series
                    (
                        new Dictionary<string, object>
                        {
                            ...
                            { "Frequency", "quarterly" },
                            { "EntityType", "TimeSeries" },
                            { "StoresRevisionHistory", true },
                            { "FirstRevisionTimeStamp", new DateTime(2018, 11, 11, 11, 0, 0, DateTimeKind.Utc) },
                            { "LastRevisionTimeStamp", new DateTime(2021, 12, 03, 0, 0, 0, DateTimeKind.Utc) },
                            { "LastValue", 47 },
                    ...
                    (
                        new Dictionary<string, object>
                        {
                            ...
                            { "EntityType", "TimeSeries" },
                            { "StoresRevisionHistory", true },
                            { "RevisionLabel", "Policy report 2021" },
                            { "FirstRevisionTimeStamp", new DateTime(2018, 11, 11, 11, 0, 0, DateTimeKind.Utc) },
                            { "LastRevisionTimeStamp", new DateTime(2021, 12, 03, 0, 0, 0, DateTimeKind.Utc) },
                            { "LastValue", 48 },
                            { "LastModifiedTimeStamp", new DateTime(2021, 12, 03, 0, 0, 0, DateTimeKind.Utc) },
                        },
                        [
                            116, 116, 119, 105, 114, 112, 115, 102, 108, 106, 108, 97, 105, 101, 105, 96, 102, 97, 99, 93, 98, 95, 97, 91, 98, 95, 98, 94, 101, 100, 100, 98, 101, 100, 102, 89, 94, 84, 84, 76,
                            80, 79, 91, 93, 104, 101, 101, 89, 96, 95, 99, 86, 93, 93, 91, 83, 90, 82, 86, 77, 86, 80, 79, 71, 76, 74, 74, 65, 74, 71, 72, 64, 69, 71, 76, 67, 71, 67, 68, 60, 63, 60, 64, 55,
                            58, 58, 58, 51, 56, 51, 53, 45, 50, 48, 49, 42, 46, 46, 48,
                        ]
                    ),
                    new Series
                    (
                        new Dictionary<string, object>
                        {
                            ...
                            { "StoresRevisionHistory", true },
                            { "FirstRevisionTimeStamp", new DateTime(2018, 11, 11, 11, 0, 0, DateTimeKind.Utc) },
                            { "LastRevisionTimeStamp", new DateTime(2022, 01, 04, 0, 0, 0, DateTimeKind.Utc) },
                            { "LastValue", 49 },
                            { "LastModifiedTimeStamp", new DateTime(2022, 01, 04, 0, 0, 0, DateTimeKind.Utc) },
                        },
                        [
                            116, 116, 119, 105, 114, 112, 115, 102, 108, 106, 108, 97, 105, 101, 105, 96, 102, 97, 99, 93, 98, 95, 97, 91, 98, 95, 98, 94, 101, 100, 100, 98, 101, 100, 102, 89, 94, 84, 84, 76,
                            80, 79, 91, 93, 104, 101, 101, 89, 96, 95, 99, 86, 93, 93, 91, 83, 90, 82, 86, 77, 86, 80, 79, 71, 76, 74, 74, 65, 74, 71, 72, 64, 69, 71, 76, 67, 71, 67, 68, 60, 63, 60, 64, 55,
                            58, 58, 58, 51, 56, 51, 53, 45, 50, 48, 49, 42, 46, 46, 48, 49,
                        ]
                    ),
                ]
            },
